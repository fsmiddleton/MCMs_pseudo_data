...
dim3=40;
%import true data
[Xtrue, Seed, Factors] = CreaMiss(4, [dim1,dim2,dim3], 0.01, 0, 0, 'RMV',42);

%consider missing data, % missing 
missing =50;
...
if export ==1
    ...
    [X, Seed, Factors] = CreaMiss(4, [dim1,dim2,dim3], 0.01, 0, missing/100, 'RMV',42);
    ...
    for i = 1:dim1
        ...
        T=array2table(reshape(X(i,:,:),dim2,dim3));
...
for miss = missing
    ...
    for n = 1:N % factors (== principal components) 
        ...
        [F,D, X_pred]=missing_indafac(X,n,mode, center,scale,1e-3,1000,method);
        Xm = nmodel(F);
        %built in metric
        fit(n,count) = D.fit;
        %own metrics 
...
coreconsistencybest = sum(c(n,:).^2)./length(c(n,:));
