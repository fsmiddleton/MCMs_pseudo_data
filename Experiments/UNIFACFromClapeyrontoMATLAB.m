...
for T = Temps' % for each excel file 
    ...
    for i = 1:size(data,1) % for each mixture - populate the mixture and excess enthalpy arrays 
        ...
        %group
        mixture(1,i) = find(strcmp(functionalgroup(ind1), func_group));
        mixture(3,i) = find(strcmp(functionalgroup(ind2), func_group));
        %chain lengths
...
for T = Temps% for each excel file
    ...
    ARD = reshape(ARD(ind_keep), [9], []);
    he_expard = reshape(he_exp(ind_keep), [9], []);
    he_predard = reshape(he_pred(ind_keep), [9],[] );
...
err_funcgroup = cell(length(func_group),length(func_group),length(Temps));
ARD_funcgroup = cell(length(func_group),length(func_group),length(Temps));
...
err_group = cell(length(func_group),length(func_group));
ARD_group = cell(length(func_group),length(func_group));
...
for func = 1:length(func_group)
    for func2 = 1:length(func_group)
        ...
        ardtemp = [];
        for i =1:counter 
            errtemp = [errtemp reshape(err_funcgroup{func,func2,i}, [9],[])]; 
            ardtemp = [ardtemp reshape(ARD_funcgroup{func,func2,i}, [9],[])];
        end 
...
[X,Y] = meshgrid(1:size(AARD,1), 1:size(AARD,1));
...
tbl = table(funcgroup1tbl', funcgroup2tbl',smse_tbl',aard_tbl', 'VariableNames', ["Functional group 1", "Functional group 2", "SMSE (J/mol)","AARD (%)" ]);
% heatmap 
