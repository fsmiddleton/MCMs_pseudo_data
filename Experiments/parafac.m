...
for f=1:Fac
    for m=1:ord-1
        if sign(sum(Factors{m}(:,f)<0)) & FixMode(m)==0
            % runs if the sum of factors less than 0 exists 
            contin=1;
            for m2 = m+1:ord
...
while (((f>crit) | (norm(connew-conold)/norm(conold)>MissConvCrit) | ConstraintsNotRight) & it<maxit)||~ nonneg_obeyed
    ...
    elseif strcmp(accel_pattern,'shifted')% this chooses harshman's every-iter way
        ...
        elseif acc==do_acc;
            ...
            for i = 1:length(DimX)
                id2 = sum(DimX(1:i).*Fac);
                ff{i} = reshape(Factors(id1+1:id2),DimX(i),Fac);id1 = id2;
            end
            model=nmodel(ff);
            model = reshape(model,DimX(1),prod(DimX(2:end)));
            %G(it)=err;
    ...
    if ~iscell(const)
        if DoWeight==0&& ~(any(const==4) || any(const==5))
            % weights not included and no constraints given
            for ii=ord:-1:1
                if ii==ord
                    i=1;
                else
                ...
                if ~FixMode(i)%no fixed modes 
                    ...
                    for j=3:ord-1
                        %only runs once for ord=4, does not run for ord<=3
                        %(ord =length(dimX))
    ...
    if ~iscell(const)
        if ~any(FixMode)&&~(any(const==2)||any(const==3))
            Sign = ones(1,Fac);
            for i=ord:-1:2% factors ord to 2 have sign convention applied 
                A=reshape(Factors(lidx(i,1):lidx(i,2)),DimX(i),Fac);% extract the loading 
                Sign2=ones(1,Fac);
                for ff=1:Fac
                    [~,sig]=max(abs(A(:,ff)));%find index of max value in the factor of the loading
                    Sign(ff) = Sign(ff)*sign(A(sig,ff));%set sign as (previous sign)*(sign of max value in factor)
                    Sign2(ff) = sign(A(sig,ff));%set sign2 = sign of the max value 
                end
                A=A*diag(Sign2);% adjust signs so that each factor has only one sign in it 
                Factors(lidx(i,1):lidx(i,2))=A(:);% reassign the loading 
            end
            A=reshape(Factors(lidx(1,1):lidx(1,2)),DimX(1),Fac);% 1st loading =A
            A=A*diag(Sign);% adjust signs in A to compensate for changes made in the other factors
            Factors(lidx(1,1):lidx(1,2))=A(:);
        end
    end
...
if ~any(FixMode)
    A=reshape(Factors(lidx(1,1):lidx(1,2)),DimX(1),Fac);
    [out,order]=sort(diag(A'*A));%eigenvalues of 1st loading 
    order=flipud(order);%reverse order
    A=A(:,order);%reorder
    Factors(lidx(1,1):lidx(1,2))=A(:);
    for i=2:ord
        B=reshape(Factors(lidx(i,1):lidx(i,2)),DimX(i),Fac);
...
if nargout>3
    ...
    % Convert to new format
    clear ff,id1 = 0;
    disp('Judge solution. Any nan in X')
    
    disp('Judge factors. Any nan in factors')
    for i = 1:length(DimX)
        id2 = sum(DimX(1:i).*Fac);
        ff{i} = reshape(Factors(id1+1:id2),DimX(i),Fac);
        id1 = id2;
