...
if MthFl==1
    ...
    if MthFl==2 || MthFl==3 
        ...
        CurDimX=DimX;
        for k=1:N
            c=Order(k);% index of the number of factors to use
            if Fac(c)>0
                
                for c1=1:c-1
                    % runs 0 times for A, then 1 for B, then 2 for C, then 3 times for D etc for the
                    % factors (A,B,C and D)
                    newi=CurDimX(c1+1);
                ...
                elseif Op==0 & (120<min(size(RedData)) & min(size(RedData))<Inf),
