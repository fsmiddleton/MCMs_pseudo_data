...
for Tempind = 1:length(Temps)
            ...
            if any(find(~isnan(X_b(:,col(k),:)))) & any(find(~isnan(X_b(row(k),:,:)))) & all(Xs(row(k),col(k),:)~=0) %ensure at least one value in each column and row
                ...
                while iter<maxiter && f>conv
                    ...
                    for c = 1:length(conc_interval)
                        ...
                        [X_predtemp,iters,F,err] = missing_parafac3(X_loop,fn,100,conv,scale,center,fillmethod,orth, mixtures,conc_interval, whichX,Temps);
    ...
    filenamesave = strcat('3wayPARAFAC-Small-concslices-LOOCV-X',whichX,'-maxiter=20000-Temp=',num2str(Temps(Tempind)), '-fillmethod=',fillmethod,'-',  date, '.mat');
    save(filenamesave) 
end %end temperature 
toc 
