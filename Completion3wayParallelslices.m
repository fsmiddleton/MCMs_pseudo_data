...
for Tempind = 1:length(Temps)
    ...
    for fn = fns 
        ...
        parfor k = 1:length(row) %filled_linear_ind must be a row vector for a for loop    
            ...
            if any(find(~isnan(X_b(:,col(k),:)))) & any(find(~isnan(X_b(row(k),:,:)))) & all(Xs(row(k),col(k),:)~=0) %ensure at least one value in each column and row
                X_pred = zeros(size(X));
                %find one part of the loss function  
                for c = 1:length(conc_interval)
                    for Tind = length(Temps)
                        Xtemp = Xs(:,:,c,Tind);
                        SS(Tind,c) = (sum(sum(Xtemp(missing_ind{Tempind}).^2)));
                    end 
                end
                SS = mean(mean(SS));
                iter = 0;
                f=2*conv;
                %completion loop
                while iter<maxiter/100 && f>conv
                    iter = iter+1;
                    SSold = SS;
                    %run missing indafac 100 times on each of the 
                    for c = 1:length(conc_interval)
                        X_loop = reshape(X_b(:,:,c,:),dim1,dim2,dim4);
                        %[X_predtemp,iters,F,err] = missing_parafac3(X_loop,fn,100,conv,scale,center,fillmethod,orth, mixtures,conc, whichX,Temps);
                        [X_predtemp,iters,F,err] = missing_indafac(X_loop,fn,100,conv,scale,center,fillmethod,orth, mixtures,conc_interval, whichX,Temps);
                        X_pred(:,:,c,:) = reshape(X_predtemp,dim1,dim2,1,dim4);
                    end 
                    
                    %check for outliers in parallel rows 
                    
                    for ind = 1:length(missing_ind)
                        tempX = X_pred(row(ind),col(ind),:,Tempind);
                        tempX(abs(tempX)>10)=mean(tempX);
                        tempScore = (tempX-mean(tempX))/std(tempX);
                        tempX(abs(tempScore)>threshold) = mean(tempX);
                        X_pred(row(ind),col(ind),:,Tempind) = tempX;
                    end 
    ...
    filenamesave = strcat('3wayINDAFAC-Small-concslices-LOOCV-X',whichX,'-maxiter=20000-Temp=',num2str(Temps(Tempind)), '-fillmethod=',fillmethod,'-',  date, '.mat');
    save(filenamesave) 
end %end temperature 
toc 
    ...
    plotcount = 0;
        ...
        filled_ind = find(~isnan(Xs));
            ...
            for i=1:length(fns)
                ...
                [X_pred,iters,F,err] = missing_parafac3(Xs,fn,maxiter,conv,scale,center, fillmethod, orth,mixtures, Tempind, whichX);
...
if import ==1
    ...
    Ts = readtable(filename, 'Sheet', num2str(Tempind));
    Xs = table2array(Ts);
    missing_ind = find(isnan(Xs));
    filled_ind = find(~isnan(Xs));
end 
ind = find(concentrations == Tempind);
...
sgtitle(strcat('Sparsity = ',num2str(Tempind)))
%% Plots of errors 
clf
...
for Tempind = concentrations
    ...
    ind = find(concentrations == Tempind);
    subplot(a,b,i)
    titlestr = strcat('Sparsity = ',num2str(Tempind));
...
%%
Xf = filldata3(Xs,'uni',mixtures,Tempind,'sign');
%% Functions 
