...
X = HE_data_sparse(1:20,1:20,1:length(conc_interval));
Xsign = sign(X);
...
for c = 1:length(conc_interval)
    ...
    Xtemp = reshape(X(:,:,c,:),dim1,dim2,dim4);
    Xscale = log(sign(Xtemp).*(Xtemp)).*sign(Xtemp);
    Xsign = sign(Xtemp);
    Xs = Xscale;
    ...
    error_LOOCV = zeros(length(fns),length(row),dim4);
    error_LOOCV2 = zeros(length(fns),length(row),dim4);
    RAD = zeros(length(fns),length(row),dim4);
    RAD2 = zeros(length(fns),length(row),dim4);
    Xm_boot=zeros( length(fns), length(filled_ind),dim4);
    Xm_boot2=zeros( length(fns), length(filled_ind),dim4);
    ...
    for fn = fns 
        ...
        parfor k = 1:length(row) %filled_linear_ind must be a row vector for a for loop    
            ...
            if any(find(~isnan(X_b(:,col(k),:)))) & any(find(~isnan(X_b(row(k),:,:)))) & all(Xs(row(k),col(k),:)~=0) %ensure at least one value in each column and row
                ...
                [X_pred,iters,F,err] = missing_indafac(X_b,fn,maxiter,conv,scale,center,fillmethod,orth, mixtures,conc, whichX,Temps);
    ...
    filenamesave = strcat('3wayINDAFAC-Small-concslices-LOOCV-X',whichX,'-maxiter=20000-conc=',num2str(c), '-fillmethod=',fillmethod,'-',  date, '.mat');
    save(filenamesave) 
end 
toc 
    ...
    plotcount = 0;
        ...
        filled_ind = find(~isnan(Xs));
            ...
            for i=1:length(fns)
                ...
                [X_pred,iters,F,err] = missing_parafac3(Xs,fn,maxiter,conv,scale,center, fillmethod, orth,mixtures, c, whichX);
...
%%
Xf = filldata3(Xs,'uni',mixtures,c,'sign');
%% Functions 
