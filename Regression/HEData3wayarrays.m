...
poly = 2; %1 for interpolation using polynomials, 2 for the RK equation, done with reduced HE data 
...
for t =1:length(Temps)
                    ...
                    if i~=j || f1~=f2
                        ...
                        if size(temp4,1)>3
                            ...
                            if poly == 1
                                ...
                                [HE_data(:,ind), uncertainty(:,ind), orderPolyfit(ind)]=RK_interp_data(temp4, conc_interval);
                            end 
    ...
    filename = strcat('TestSMALLreducedHEMatrix14June',num2str(T),'.xlsx');
...
function [HE, uncertainty, orderPolyfit]=RK_interp_data(data, conc_interval)
    % Interpolate data for use in the matrices using the Redlich Kister
    % equation:
    % HE = A+B(2x-1)+C(2x-1)^2+D(2x-1)^3+E(2x-1)^4+F(2x-1)^5
    % Inputs specify the mixture to be captured and the concentration
    % interval for which to create interpolate points
    ...
    HE_original = data.ReducedHE;
    comp = data.Compositioncomponent1;
    numberofpoints = length(comp);
    % remove the 0 and 1 data points 
    ind_keep = find(HE_original ~=0);
    HE_new = HE_original;
    comp_new = comp;
    if ~ind_keep
        comp_new(numberofpoints+1,1)=0;
    ...
    if numberofpoints>6
        ...
        maxOrder=numberofpoints-1;
    end 
    error = zeros(maxOrder-1,1);
    ind = 1;
    for i =2:maxOrder
        %Y=XB, B=X\Y, Y=HE, X= (2x-1)^i, B=constants
        Y=HE_new; % column n-by-1
