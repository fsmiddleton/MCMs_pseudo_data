...
[No{1:ndims(X)}]   = Clean(X);% indices of slabs without only missing values (slabs contain some missing and not missing data)
X                  = X(No{:});%only use the slabs without only missing values 
dimX               = size(X);
Xfilled = reshape(filldata(X),dimX(1),prod(dimX(2:end)));
%Find missing values
Miss = isnan(X(:));
...
while ~any(ConvCrit)
   ...
   it(2) = it(2) + 1;    
    Xnew = nmodel(Factors);
    Xnew = reshape(Xnew, dimX(1),prod(dimX(2:end)));
    threshold = 7;
    if any(any(abs(Xnew)>threshold))
        [rowind,colind] = find(abs(Xnew)>threshold);
        for index = 1:length(rowind)
            Xnew(rowind(index),colind(index)) =(mean(Xnew(rowind(index),:))+(mean(Xnew(:,colind(index))))')/2; 
            %disp(Xnew(rowind(index),colind(index))) 
        end 
        Xnew(abs(Xnew)>threshold)=0;%set outliers that still remain equal to 0
        Xnew = reshape(Xnew,dimX);
        [Factors,~,~,~] = parafac(Xnew, F);
    end
   ...
   if ~it(1) %Calculate indexes for position of missing and non-zeros for Jacobian
       %if it(1) ==0, find this: started at 0 and only runs for the first
       %iteration 
   ...
   while Do_It && ~any(ConvCrit) %Begin the internal loop
      ...
      [Psi,CFlag]  = chol(Psi);      %Cholesky factor; psi2*psi2'=psi
      if ~CFlag                      %Choleski is positive definite, all positive eigen values - what we want 
   ...
   Factors = Fac_Up;
    ...
    %Factors corresponding to this model 
...
% Last modified: September 3rd, 2004 
%
%comments given by FS Middleton for a 3-way array

if ~exist('Opt','var')
   Opt = false;
end
dimX = size(X);%[I,J,K]==[n1,n2,n3] or other for a different size matrix
Nel  = prod(dimX);%scalar
F    = size(Factors{1},2);%number of factors 
JtJ  = [];%size Jacobian = (IJK)x(I+J+K)F
Gr   = [];
if ~exist('XSort','var') | isempty(XSort) 
   ...
   Xs  = reshape(1:Nel,dimX);% indices of all values in the shape of X, in order. Goes down columns in the first slice, then second slice etc  
   Xm = vec(isnan(X));%missing values array of data  
   ...
   for m = 1:ndims(X) %goes through dimensions of X
      %permute(X, dimorder))
      x   = permute(Xs,[m, 1:m-1, m+1:length(dimX)]);%reorder vector to have m dimension first
      x   = repmat(x(:),1,F);% repeat copies of the vector of the permuted array with F of the columns next to each other  
      ord = [ndims(X):-1:m+1, m-1:-1:1];%order of dimenions from biggest to smallest, without m
      s   = prod(dimX(ord));%all dims besides m multiplied 
      fo  = repmat(0:F-1,s,1) * Nel;%repeats vector in rows of 1:F, s times, and multiply it all by the number of indices in the array X
      for i = 1:dimX(m)
         %i = size of the dimension m on the last iteration 
         a            = repmat([i:dimX(m):Nel]',1,F);% a is a column repeated. values increase in increments of dimX(m) 
         b            = ((i - 1) + sum(dimX(1:m-1))) * F * Nel;%0 for the first dimension 
         %indices 
         rs           = (i - 1) * s * F + 1;
         re           = i * s * F;
   ...
   [JtJ(:,1),JtJ(:,2)] = ind2sub([Nel,sum(dimX) * F],Ind(:));%convert linear indices to subscripts 
   %JtJ columns have indices of all values
   % [row and column subscripts] = ind2sub(size Jacobian, Indices)
   Ind                 = true(Nel * F * ndims(X),1);
   if any(Xm)
      %Handle missing values or 0 weights for any element
      Ind      = false(Nel * F * ndims(X),1);%zeros
      Gr       = Xm;
      [Nil,a]  = sort(JtJ,1);
      a        = reshape(a(:,1),ndims(X)*F,Nel);%indices of the sorted 1st dimension of JtJ are reshaped to ndims(X)*F by IJK
      a(:,Xm)  = [];%remove missing values 
      Ind(a)   = true;
   end
   if all(Ind)%if all elements are non-zero or true - no missing values 
   ...
   if ~isempty(Ind)
      ...
      JtJ      = uint32(JtJ(Ind,:));%JtJ now holds 
      JtJ(:,3) = Ind;
   else
      JtJ = uint32(JtJ);
   end
   ...
   %actually finds the Jacobian 
   %second use of the function 
   Fix = logical([]);
   for m = 1:ndims(X) %for each dimension of X
      t = ones(1,F);% vector of ones for size of number of factors 
      ord = [ndims(X):-1:m+1, m-1:-1:1];%all dimensions besides m 
      for n = 1:length(ord)
         t = krb(t,Factors{ord(n)});%kharti rao product 
      end
      JtJ((m - 1) * Nel * F + 1:m * Nel * F) = vec(kron(ones(1,dimX(m)),t));% vector of the kronecker product 
...
% 
% Calls in the n-way toolbox: dtld, kr(krb), ini, missmult, nmodel, normit, nshape, parafac
% 
% 
% Author: 
% Giorgio Tomasi 
...
% X = filled matrix 
    if size(X)>2
        ...
        for p = 1:size(X,3)
            [m,n]=size(X(:,:,p));
            missing_ind = (isnan(X(:,:,p)));
            [i, j]=find(isnan(X(:,:,p)));% returns rows and columns with nonzero elements
            X_filled=X(:,:,p);
            ...
            for k =1:length(i) 
                X_filled(i(k),j(k),p)=(mean_row(i(k))+mean_col(j(k)))/2;
            end  
        end 
    else %2-way 
        [m,n]=size(X);
