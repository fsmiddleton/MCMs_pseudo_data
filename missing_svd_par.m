function [S,V,D,St,X_predall, AllSVs, iterations]=missing_svd_par(X,fn,center,scale,conv,max_iter, use_missing,fillmethod,mixtures,whichX,conc,T)
    ...
    if any(isnan(X(:,:,1))) % there is missing data 
        ...
        while iter<max_iter && f>conv
            ...
            if mod(iter,55)==0 || fold<f % every 55 iterations check for outliers 
